...
export default function VisitasPage() {
  ...
  return (
    <AppShell
      active="/visitas"
      title="Visitas de inspeccion"
      subtitle="Registro de visitas de autoridades sanitarias"
      width="wide"
    >
      <div className="grid gap-4 lg:grid-cols-2 lg:gap-6 lg:items-start">
        <SectionCard title="Nueva visita">
          ...
          <Field label="Nombre autoridad sanitaria">
            <TextInput
              value={form.autoridadSanitaria}
              onChange={(e) => setForm({ ...form, autoridadSanitaria: e.target.value })}
            />
          </Field>
          <Field label="Concepto emitido">
            <TextArea
              value={form.conceptoEmitido}
              onChange={(e) => setForm({ ...form, conceptoEmitido: e.target.value })}
            />
          </Field>
          <Field label="Nombre del funcionario">
            <TextInput
              value={form.funcionarioNombre}
              onChange={(e) => setForm({ ...form, funcionarioNombre: e.target.value })}
            />
          </Field>
          <Field label="Cargo del funcionario">
            <TextInput
              value={form.funcionarioCargo}
              onChange={(e) => setForm({ ...form, funcionarioCargo: e.target.value })}
            />
          </Field>
        ...
        <SectionCard title="Historial de visitas">
          {visitas.length === 0 ? (
            <p className="text-sm text-[var(--muted)]">No hay visitas registradas.</p>
          ) : (
              ...
              {visitas.map((v) => (
                <div key={v.id} className="rounded-xl border border-[var(--border)] p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="font-semibold text-[var(--deep)]">
                        {v.autoridadSanitaria || "Sin autoridad"}
                      </p>
                    ...
                    <button
                      ...
                      className="text-xs text-[var(--alert)]"
                    >
                      Eliminar
                    </button>
                  </div>
                  ...
                  <p className="mt-1 text-xs text-[var(--muted)]">
                    {v.funcionarioNombre} - {v.funcionarioCargo}
                  </p>
                </div>
              ))}
            </div>
